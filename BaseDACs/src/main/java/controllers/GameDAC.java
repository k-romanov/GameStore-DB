...
public class GameDAC extends DAC {
    ...
    public static ResultSet getGameList(int start, int end) throws SQLException {
        return DAC.getSet(String.format("SELECT game_id from Games ORDER BY game_id LIMIT %d,%d", start, end));
    }

    public static ResultSet getGameListByQuery(String name, int start, int end) throws SQLException {
        return DAC.getSet(String.format("SELECT game_id from Games where (LOCATE(%s, game_title) > 0) ORDER BY game_id LIMIT %d,%d", name, start, end));
